...
})
{
    ...
    async function insertAuthDetails({ empid, accesstoken, expiretime, sessionid, ipaddress, useragent, city, state, country })
    {
        let result = await pool.query(`INSERT INTO ${authTable} (empid, sessionid, accesstoken, expiretime, ipaddress, useragent, city, state, country) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING empid`, [empid, sessionid, accesstoken, expiretime, ipaddress, useragent, city, state, country]);       
    ...
    {
        console.log("session id in get-expire-time", sessionid);

        let result = await pool.query(`SELECT expiretime FROM ${authTable} WHERE sessionid = $1`, [sessionid]);

        return result.rows[0].expiretime;
    }
    async function getAllSessions({ empid, columnToSort, sortingOrder })
    {
        let result = await pool.query(`SELECT * FROM ${authTable} WHERE empid = $1 ORDER BY ${columnToSort} ${sortingOrder}`, [empid]);
        return result.rows;
    }
